...
skip:
        ...
        while (retry_attempts++ < RETRY_MAX) {
                ...
                if (stat1.st_ino == stat2.st_ino) {
                        ...
                        st = unlink(lockfn_link);
                        if (unlikely(st == -1)) {
                                LOG("error removing lock file %s", lockfn_link);
                        }
                        retry_attempts = 0;
                        XSLEEP;
                        status = LOCK_EINODE;
...
int unlock(char *fn_to_unlock, char *uuid)
{
        ...
        if (fd == -1) {
                /* no lock folds to successful unlock */
                if (errno == ENOENT)
                        status = LOCK_OK;
                else 
                        status = LOCK_EOPEN;
...
int lock_delta(char *fn_to_check)
{
        ...
        if (unlikely(!lockfn)) { result = LOCK_ENOMEM; goto finish; }
        if (lstat(lockfn, &statbuf) == -1) { result = LOCK_ESTAT; goto finish; }
